...
void getsyndrome(int8_t syndrome[], int8_t codeword[]){
  ...
  while(i != n-k+1){
      ...
      while(j != n){
            ...
            #elif defined constant_time_RS_15_13_1
            syndrome[i] ^= alpha_to[mod15((codeword[j]+i*j) & (-(1^(uint16_t)codeword[j] >> 15)))] & (-(1 ^ ((uint16_t)codeword[j]) >> 15)); 
            ...
            if(codeword[j] != -1){
                syndrome[i] ^= alpha_to[mod(codeword[j]+i*j, n)];
            }
    ...
    if(l[u]<=t){
        ...
        if (count==l[u]){
            ...
            while(i != n){
                ...
                //constant-time protection
                #if defined(constant_time_RS_31_29_1) || defined(constant_time_RS_31_27_2) || defined constant_time_RS_15_13_1 || defined constant_time_RS_63_49_7
...
int main(){
    ...
    while(ite != iterationtime * exectime){
        ...
        //Received from TX
        int8_t ss[2*t+1]; //the last 2*t elements are received from TX, the first is 0 by default
