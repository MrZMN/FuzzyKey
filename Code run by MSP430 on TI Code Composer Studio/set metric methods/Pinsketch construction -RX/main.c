...
#define constant_time   //constant-time protection

//Pinsketch Configurations//the settings are in Pinsketch_n_s_t format. The error tolerance is t/2s.

//error tolerance = 2%
#define Pinsketch_255_50_2

//error tolerance = 5%
#define Pinsketch_255_20_2

//error tolerance = 10%
#define Pinsketch_255_10_2


#ifdef Pinsketch_255_50_2
...
#elif defined Pinsketch_255_20_2
...
#elif defined Pinsketch_255_10_2
...
void bubble_sort(uint8_t array[], uint8_t len){
    ...
    while(i != len-1){
        ...
        while(j != len-1-i){
            ...
            uint8_t big = (array[j] & (-(1 ^ ((uint16_t)diff >> 15)))) ^ (array[(uint8_t)(j+1)] & (-(1 ^ ((uint16_t)-diff >> 15))));
            uint8_t small = (array[j] & (-(1 ^ ((uint16_t)-diff >> 15)))) ^ (array[(uint8_t)(j+1)] & (-(1 ^ ((uint16_t)diff >> 15))));
            array[j] = small;
            array[(uint8_t)(j+1)] = big;
    ...
    while(i != 2*t+1){
        ...
        if(mod(i, 2) != 0){
            ...
            if(syndrome[divi(i,2)] == -1){
                ...
                syndrome[i] = mod255(syndrome[divi(i,2)]*2);
...
int main(){
    ...
    while(ite!= iterationtime * exectime){
        ...
        int16_t ssTX[t];      //stores secure sketch at TX side - received from TX
