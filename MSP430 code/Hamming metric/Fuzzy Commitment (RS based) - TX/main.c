...
int main(){
    ...
    while(ite!= iterationtime * exectime){
        ...
        uint8_t randval[32];             //RNG of MS430 - length should be set as multiples of 16 >= k
        rng_generateBytes(randval, 32);

        uint8_t key[k];                  //amount of secret transmitted per execution
        i = 0;
        while(i != k){
            key[i] = randval[i] & n;
            i++;
        ...
        uint8_t ps[n];                      //extracted PS bit string
        ...
        //use ps string to conceal the codeword
        i = 0;
        while(i != n){
