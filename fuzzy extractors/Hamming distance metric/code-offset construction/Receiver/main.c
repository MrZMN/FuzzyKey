...
// This is RX of a 'code-offset' fuzzy extractor constructed by a (15,11) RS-code
...
int main(){
  ...
  ps[n/2] ^= 1;
  // ps[n-1] ^= 1;
  uint8_t ss[n] = {60, 65, 5, 109, 240, 28, 74, 51, 210, 1, 83, 48, 226, 187, 231};    //received from TX
  uint8_t randStr[n] = {105, 124, 121, 231, 41, 138, 127, 187, 118, 161, 198, 208, 21, 235, 20}; //received from TX
  uint8_t encodednonce[n];  //the encoded nonce (nonce generated at TX side)
  uint32_t key[8];          //the final key

  //codeword (of the nonce) = secure sketch xor ps. If ps values at TX/RX sides mismatch, there'll be errors in codeword
  i = 0;
  while(i != n){
    encodednonce[i] = ss[i] ^ ps[i];
    i++;
  }

  //decode the codeword
  decode_m_4(encodednonce);
