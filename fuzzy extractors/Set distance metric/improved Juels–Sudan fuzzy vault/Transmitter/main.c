...
#define n 255   //universe size - 1
//The max tolerable mismatch rate = t/2s
//The initial entropy is log(n+1,s) with base 2, where (n+1,s) stands for the number of combinations of taking random s elements from n+1 elements without putting back
...
//The TX of Improved Juels-Sudan construction of fuzzy extractor
int main(){

    uint8_t i;

    //TX's set generated from PS measurements. Each element should be different from each other (from 0 to n-1). We use 0~s-1 to simulate it.
