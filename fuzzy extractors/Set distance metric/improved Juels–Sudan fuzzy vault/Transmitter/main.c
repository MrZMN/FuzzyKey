...
//The TX of Improved Juels-Sudan fuzzy vault
//The principle is: TX constructs a poly based on all PS measurements. TX devides the poly into two parts phigh(x) + plow(x), and transmits coeffs of phigh(x) to RX
...
int main(){
    ...
    uint8_t ps[pslength];                 //stores the 'pslength' num of PS values measured by the TX  - poly form
    int ss[t];                            //stores the secure sketch to be transmitted to RX - index form
    uint8_t wholeps[opernum*pslength];    //stores all the PS values measured by the TX. In this example we need '2*pslength' num of ps values, but we do this for two times because of hardware limit.
    int wholess[opernum*t];               //stores all the secure sketch. In this example we need '2*t' num of secure sketch, but we do this for two times because of hardware limit.
    uint8_t randStr[opernum*pslength] ;   //stores a random string used in a strong extractor
    ...
    for(j = 0; j < opernum; j++){
        ...
        for(i = 0; i < t; i++){
            ss[i] = -1;
            ss[i] = comb(ps, pslength, i+1);
        }

        for(i = 0; i < pslength; i++){
            wholeps[i+j*pslength] = ps[i];
            ps[i] = 0;
        }
        for(i = 0; i < t; i++){
            wholess[i+j*t] = ss[i];
    ...
    printf("PS values at TX: (copy it to Receiver)\n");
    for(i = 0; i < opernum*pslength; i++){
        printf("%d, ", wholeps[i]);
    }
    printf("\n");
    printf("Secure sketch: (copy it to Receiver)\n");
    for(i = 0; i < opernum*t; i++){
        printf("%d, ", wholess[i]);
    }
    printf("\n");
    ...
    //////the next step is to send the 20 byte randStr[] and 4 byte ss[] to RX
