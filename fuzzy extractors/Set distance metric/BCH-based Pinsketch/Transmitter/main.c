...
int isinset(uint8_t, uint8_t [], int);
...
int main(){
	...
	uint8_t ps[nummeasure];		//stores the ps measurements at TX side - poly form. values should be different from each other
	int ss[t];				//stores the secure sketch to be transmitted to RX	- poly form
	uint8_t randStr[nummeasure];			//stores the ps measurements
	uint32_t key[8];			//the final key

	//Randomly generate the ps values - assume each value is 1 byte, and we want 'nummeasure' values
	for(i = 0; i < nummeasure; i++){	
		randval = rand()%256;	//each ps value from GF(2^8), and they're not the same 
		while(isinset(randval, ps, nummeasure)){
			randval = rand()%256;
		}
		ps[i] = randval; 
	}
	...
	//////The next step is to send the t-byte secure sketch to RX
