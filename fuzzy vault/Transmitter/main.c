...
#define numchaffpoint 100	//the num of chaff points added - for each chaff point (x,y), 1. x isn't the same as the x-coordinates of above legit points 2. (x,y) isn't on the key polynomial
...
//This is the TX of fuzzy vault
int main(){

	srand((unsigned)time(NULL));	//For random number generation

	register int i, j, randval;

	int key[polydegree + 1];			//(part of) the 128-bit key, each element in this array is from GF(2^8)
	int legitps[numlegitpoint];			//legit ps measurements at TX side
	int legity[numlegitpoint];			//y-coordinates when projecting legit ps values on the poly key[] 
	int chaffps[numchaffpoint];			//random fake ps measurements (should be different from the ones in legitps[])
	int chaffy[numchaffpoint];			//random fake y values (the chaff points (x,y) can not be on poly key[]) 
	int legitpointpos[numlegitpoint];	//stores the random positions of the legit points - to mix the points 
	int pointsx[totalnum];				//stores the mixed x-coordinates (mixed for security)
	int pointsy[totalnum];				//stores the mixed y-coordinates (mixed for security)
	
	int wholekey[opernum*polydegree+opernum];		//may need to agree the whole 128-bit key for several times, due to the hardware constraints
	...
	for(j = 0; j < opernum; j++){
		...
		for(i = 0; i < polydegree+1; i++){
			key[i] = rand()%256;
			wholekey[i+j*polydegree+j] = key[i];
		}
    ...
    //////Because of the embedded system memory limit, it's always impossible to generate all the points at the same time, then send them all to RX. Should divide the whole process into pieces. 
	}
